...
def main():
    ...
    for ticker in tickers:
        try:
            ...
            print(f"🔎 {ticker} X shape: {X.shape}, mean: {X.mean():.6f}, std: {X.std():.6f}")
            ...
            print(f"🔮 Prediction for {ticker} on {next_trading_day}: {float(y_pred):.6f} (log return from {last_date.date()})")
